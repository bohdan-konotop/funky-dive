...
export class NavigationService {
  ...
  private findNextCellIndex = {
    ArrowUp: (currentRow: number, currentCol: number) =>
      this.findCellIndexUp(currentRow, currentCol),
    ArrowRight: (currentRow: number, currentCol: number) =>
      this.findCellIndexRight(currentRow, currentCol),
    ArrowDown: (currentRow: number, currentCol: number) =>
      this.findCellIndexDown(currentRow, currentCol),
    ArrowLeft: (currentRow: number, currentCol: number) =>
      this.findCellIndexLeft(currentRow, currentCol),
  };
  ...
  public handleNavigation = (e: KeyboardEvent) => {
    if (this.isArrowKey(e)) {
      this.focusNextCell(
        ...
        this.focusedCellIndices.column
      );
    }
  };
  ...
  private focusNextCell(
    ...
    const nextCellIndex = this.findNextCellIndex[event.key as Direction](
      currentRow,
      currentCol
    );
  ...
  private findCellIndexRight(currentRow: number, currentCol: number): number {
    const nextIndexInRow = this.getNextAvailableIndex(
      this.sudoku[currentRow],
      currentCol
    );
  ...
  private findCellIndexLeft(currentRow: number, currentCol: number): number {
    const previousIndexInRow = this.getPreviousAvailableIndex(
      this.sudoku[currentRow],
      currentCol
    );
  ...
  private findCellIndexUp(currentRow: number, currentCol: number): number {
    const column = this.getColumnFromSudoku(currentCol);
    const previousIndexInColumn = this.getPreviousAvailableIndex(
      column,
      currentRow
    );
  ...
  private getNextAvailableIndex = (
    arr: ArrayOf9Elements<Cell>,
    currentIndex: number
  ): number => {
    return currentIndex + 1 < arr.length ? currentIndex + 1 : 0;
  };

  private getPreviousAvailableIndex = (
    arr: ArrayOf9Elements<Cell>,
    currentIndex: number
  ): number => {
    return currentIndex - 1 >= 0 ? currentIndex - 1 : arr.length - 1;
  };
